...
export default function RecipePage({ recipe, ingredients, steps }: { recipe: Recipe; ingredients: Ingredient[]; steps: Step[] }) {
  if (!recipe) return <div className="container"><p>Rețetă negăsită.</p></div>;
  return (
    <div className="container">
      <h1>{recipe.title_ro}</h1>
      {recipe.description_ro && <p>{recipe.description_ro}</p>}
      <div className="card" style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:16}}>
        <div>
          <h3>Valori</h3>
          <ul>
        ...
        <div>
          <h3>Ingrediente</h3>
          <ul>
            {ingredients.map(i => (<li key={i.id}>{i.name} — {i.quantity ?? ""}</li>))}
          </ul>
        </div>
        <div>
          <h3>Pași</h3>
          <ol>
            {steps.sort((a,b)=>a.step_no-b.step_no).map(s => (<li key={s.id}>{s.text_ro}</li>))}
          </ol>
        </div>
      </div>
    </div>
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const { data: steps } = await supabase.from("recipe_steps").select("*").eq("recipe_id", id);
